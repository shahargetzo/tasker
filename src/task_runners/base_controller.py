...
from src.common.databases_struct import process_config
from src.common.service_providers.data_provider import DataProvider
from src.common.service_providers.queue_provider import QueueProvider, QueueException
    ...
    def start_listening(self):
        while True:
            ...
            if not process_status or process_status[0].get(process_config.key_status,
                ...
                self.logger.warning(
                    f'process {self.name} has no row in {process_config.table_name} '
                    f'or status is not active, killing self')
                self.kill_self()
            try:
